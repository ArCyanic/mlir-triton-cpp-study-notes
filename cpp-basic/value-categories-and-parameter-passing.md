...
- [1. 贯穿场景：构建编译器 IR 算子节点](#1-贯穿场景构建编译器-ir-算子节点)
- [2. 表达式与值类别体系](#2-表达式与值类别体系)
  - [2.1 从“左值/右值”到五大值类别体系](#21-从左值右值到五大值类别体系)
  - [2.2 两个核心判定维度：Identity 与 Movability](#22-两个核心判定维度identity-与-movability)
  - [2.3 典型表达式的值类别归属推导](#23-典型表达式的值类别归属推导)
- [3. 形参正交空间与 5×6 绑定矩阵](#3-形参正交空间与-56-绑定矩阵)
  - [3.1 形参类型空间的正交分解（$2 \times 3 = 6$ 种形态）](#31-形参类型空间的正交分解2-times-3--6-种形态)
  - [3.2 实参形态与 $5 \times 6$ 完整绑定能力与权限判定矩阵](#32-实参形态与-5-times-6-完整绑定能力与权限判定矩阵)
  - [3.3 引用绑定的三大底层不变式规则](#33-引用绑定的三大底层不变式规则)
  - [3.4 奇特形态剖析：为什么 `const T&&` 是反模式？](#34-奇特形态剖析为什么-const-t-是反模式)
  - [3.5 典型传参模式与 Sink Parameter 模式](#35-典型传参模式与-sink-parameter-模式)
  - [3.6 编译器重载决议匹配优先级（Overload Resolution Ranking）](#36-编译器重载决议匹配优先级overload-resolution-ranking)
- [4. 移动语义原理与类型系统退化](#4-移动语义原理与类型系统退化)
  - [4.1 `std::move` 的本质：纯编译期静态类型转换](#41-stdmove-的本质纯编译期静态类型转换)
  - [4.2 右值引用变量在函数内部为何是“左值”？](#42-右值引用变量在函数内部为何是左值)
  - [4.3 物理执行剖析：指针所有权窃取（Pointer Stealing）](#43-物理执行剖析指针所有权窃取pointer-stealing)
  - [4.4 `const T` 导致的移动静默退化为拷贝（Silent Fallback to Copy）](#44-const-t-导致的移动静默退化为拷贝silent-fallback-to-copy)
- [5. 拷贝消除与返回值优化（RVO/NRVO）](#5-拷贝消除与返回值优化rvonrvo)
  - [5.1 C++17 保证拷贝消除（Guaranteed Copy Elision）与 Prvalue 本质](#51-c17-保证拷贝消除guaranteed-copy-elision与-prvalue-本质)
  - [5.2 为什么在 `return` 局部变量时写 `std::move` 是反优化（Pessimization）？](#52-为什么在-return-局部变量时写-stdmove-是反优化pessimization)
- [6. 万能引用与完美转发](#6-万能引用与完美转发)
  - [6.1 万能引用（Universal/Forwarding Reference）与引用折叠](#61-万能引用universalforwarding-reference与引用折叠)
  - [6.2 `std::forward<T>` 的条件静态类型转换机制](#62-stdforwardt-的条件静态类型转换机制)
- [7. 传参选型矩阵与决策流](#7-传参选型矩阵与决策流)

---

## 1. 贯穿场景：构建编译器 IR 算子节点
...
### 2.2 两个核心判定维度：Identity 与 Movability
...
### 3.1 形参类型空间的正交分解（$2 \times 3 = 6$ 种形态）
...
### 3.2 实参形态与 $5 \times 6$ 完整绑定能力与权限判定矩阵
...
### 3.6 编译器重载决议匹配优先级（Overload Resolution Ranking）
...
### 4.3 物理执行剖析：指针所有权窃取（Pointer Stealing）
...
### 4.4 `const T` 导致的移动静默退化为拷贝（Silent Fallback to Copy）
...
## 5. 拷贝消除与返回值优化（RVO/NRVO）

### 5.1 C++17 保证拷贝消除（Guaranteed Copy Elision）与 Prvalue 本质
...
### 5.2 为什么在 `return` 局部变量时写 `std::move` 是反优化（Pessimization）？
...
### 6.1 万能引用（Universal/Forwarding Reference）与引用折叠
...
针对编译器与高性能 C++ 开发中的接口设计，遵循以下**终极选型法则**：
